...
#define IBAT_NO_CHARGER    0
#define GAUGE_DEVIATION_PASS_COUNT    2
static ssize_t gauge_deviation_result_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	char page[256] = { 0 };
	int len = 0;
	int plat_vbat = 0;
	...
	int ibat_deviation = 0;
	u8 cp_adc_reg = 0;
	int final_deviation = 0;
	int retry = 5, pass_cnt = 0;
	...
	while (retry > 0) {
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
			...
			ibat_deviation = (gauge_ibat > 0) ? gauge_ibat - IBAT_NO_CHARGER : IBAT_NO_CHARGER - gauge_ibat;
			...
			chg_err("%s cp_vbat=%d,gauge_bat=%d,gauge_ibat=%d,retry:%d\n",
						__func__, cp_vbat, gauge_bat, gauge_ibat, retry);
			if (cp_vbat != 0 && gauge_bat != 0 && vbat_deviation < DEVIATION_NUM) {
				if (ibat_deviation < IBAT_DEVIATION_NUM)
					pass_cnt++;
				else
					pass_cnt = 0;
				...
				msleep(100);
				retry--;
				if (pass_cnt >= GAUGE_DEVIATION_PASS_COUNT) {
					final_deviation = 1;
					chg_err("%s detection pass\n", __func__);
					break;
				}
			} else {
				msleep(100);
			...
			gauge_bat = oplus_gauge_get_batt_mvolts();
			gauge_ibat = oplus_gauge_get_batt_current();
			ibat_deviation = abs(gauge_ibat - 0);
			vbat_deviation =
	...
	chg_err("%s vbat_deviation=%d,ibat_deviation=%d,final_deviation=%d,retry:%d\n",
		__func__, vbat_deviation, ibat_deviation, final_deviation, retry);

	len = sprintf(page, "%d", final_deviation);
	if (len > *off) {
		len -= *off;
	} else {
		len = 0;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
static const struct file_operations gauge_deviation_result_proc_fops = {
	.read = gauge_deviation_result_read,
	.llseek = noop_llseek,
};
#else
static const struct proc_ops gauge_deviation_result_proc_fops = {
	.proc_read = gauge_deviation_result_read,
	.proc_lseek = noop_llseek,
};
#endif
...
static int init_charger_proc(struct oplus_chg_chip *chip)
{
	...
	prEntry_tmp = proc_create_data("gauge_deviation_result", 0444, prEntry_da, &gauge_deviation_result_proc_fops, chip);
	if (prEntry_tmp == NULL) {
		ret = -1;
		chg_debug("%s: Couldn't create gauge_deviation_result proc entry, %d\n", __func__, __LINE__);
	}
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	rc = oplus_tp_panel_notifier_register_client(&chip->chg_fb_notify);
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (0/*(get_eng_version() == HIGH_TEMP_AGING) || oplus_is_ptcrb_version()*/)
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (0/*get_eng_version() == HIGH_TEMP_AGING || oplus_is_ptcrb_version() || get_eng_version() == AGING ||
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (event == OPLUS_PANEL_EVENT_BLANK) {
		...
		} else if (blank == OPLUS_PANEL_POWER_OFF) {
			pr_err("%s led_off\n", __func__);
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (0/*(get_eng_version() == HIGH_TEMP_AGING || get_eng_version() == AGING || oplus_is_ptcrb_version()) &&
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
	if (0/*chip->charger_exist && ((get_eng_version() == HIGH_TEMP_AGING) || oplus_is_ptcrb_version())*/) {
		chg_count++;
...
#ifndef CONFIG_DISABLE_OPLUS_FUNCTION
		if (1/*get_eng_version() != HIGH_TEMP_AGING && !oplus_is_ptcrb_version()*/) {
...
static void oplus_chg_check_abnormal_adapter(int vbus_rising)
{
	...
	if (oplus_chg_fg_package_read_support()) {
		...
		if (!vbus_rising) {
			pre_pd_svooc = chip->pd_svooc;
			if (chip->chg_ops && chip->chg_ops->get_cc_value)
				pre_is_abnormal_adapter = chip->is_abnormal_adapter && chip->chg_ops->get_cc_value();
			else
				pre_is_abnormal_adapter = chip->is_abnormal_adapter;
			chip->is_abnormal_adapter = false;
