...
extern int oplus_check_cc_mode(void);
#ifndef CONFIG_OPLUS_CHARGER_MTK
extern int oplus_get_cc_value(void);
#endif
...
#define OPLUS_BC12_RETRY_TIME        300
...
static void sc6607_bc12_redetect_work_func(struct work_struct *work)
{
	if (!g_chip) {
		pr_err("%s return\n", __func__);
		return;
...
static bool sc6607_check_rerun_detect_chg_type(struct sc6607 *chip, u8 type)
{
	...
	if (chip->bc12.first_noti_sdp && (type == SC6607_VBUS_TYPE_SDP || type == SC6607_VBUS_TYPE_CDP)) {
		pr_err("%s sdp/cdp, redetect\n", __func__);
		schedule_delayed_work(&chip->bc12_redetect_work, msecs_to_jiffies(OPLUS_BC12_RETRY_TIME));
		return true;
	}
	chip->bc12_done = true;
	return false;
	...
	struct sc6607 *chip, enum power_supply_type *type)
{
	...
	if (!chip->soft_bc12) {
		...
		if (hw_bc12_done) {
			...
			bc12_try_count++;
			pr_err("bc12_try_count[%d] reg_type:0x%x\n", bc12_try_count, vbus_stat);
		} else {
...
/********* workaround: Octavian needs to enable adc end *********/
		...
		cancel_delayed_work_sync(&chip->bc12_redetect_work);
...
static int sc6607_dpdm_irq_handle(struct sc6607 *chip)
{
	...
	if (!chip->power_good) {
		pr_info("adapter removed, return\n");
		chip->soft_bc12_type = SC6607_VBUS_TYPE_NONE;
		chip->chg_type = CHARGER_UNKNOWN;
		chip->oplus_chg_type = POWER_SUPPLY_TYPE_UNKNOWN;
		sc6607_set_usb_props_type(chip->oplus_chg_type);
...
static int sc6607_vooc_irq_handle(struct sc6607 *chip)
{
	...
	pr_err("%s work_mode:%d\n", __func__, oplus_voocphy_mg->cp_work_mode);
	...
	case CP_WORKMODE_PPS:
		...
		break;
	default:
		sc6607_voocphy_reset_voocphy(oplus_voocphy_mg);
		break;
	}
...
static int oplus_sc6607_hardware_init(void)
{
	...
	if (!strcmp(g_chip->chg_dev_name, "primary_chg")) {
		...
		if (atomic_read(&g_chip->charger_suspended) == 1)
			...
			sc6607_set_input_current_limit(g_chip, SC6607_DEFAULT_IBUS_MA);
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
	chg = &chg_chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->pd_sdp && chg_chip->usb_psy) {
		power_supply_changed(chg_chip->usb_psy);
		return POWER_SUPPLY_TYPE_USB_PD_SDP;
	}
#endif
...
static bool oplus_sc6607_check_chrdet_status(void)
{
	...
	if (oplus_vooc_get_fastchg_started()|| oplus_vooc_get_fastchg_dummy_started() ||
        ...
        (g_chip->is_real_charger && chg_chip->camera_on))
		return true;
...
static int oplus_sc6607_get_charger_subtype(void)
{
	...
	pd_type = oplus_sc6607_get_pd_type();
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
	.get_cc_value = oplus_get_cc_value,
#endif
...
static int sc6607_voocphy_init_device(struct oplus_voocphy_manager *chip)
{
	...
	if (g_chip->chip_id == SC6607_1P0_CHIP_ID)
		...
		sc6607_voocphy_write_byte(chip->client, SC6607_REG_VAC_VBUS_OVP, 0x31); /*Vac ovp 11V	 Vbus_ovp 10V*/
...
static int sc6607_voocphy_svooc_ovp_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	ret = sc6607_field_write(g_chip, F_VAC_OVP, 0x03);
...
static int sc6607_voocphy_svooc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	if (g_chip->chip_id == SC6607_1P0_CHIP_ID)
		...
		ret = sc6607_field_write(g_chip, F_VAC_OVP, 0x03); /*VAC_OVP:11v VBUS_OVP:10v*/
	ret = sc6607_field_write(g_chip, F_VBUS_OVP, 0x01);
...
static int sc6607_voocphy_pdqc_hw_setting(struct oplus_voocphy_manager *chip)
{
	...
	if (g_chip->chip_id == SC6607_1P0_CHIP_ID)
		...
		ret = sc6607_field_write(g_chip, F_VAC_OVP, 3); /*VAC_OVP:11v VBUS_OVP:10v*/
...
static int sc6607_cp_config_sc_mode(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_config_sc_mode not get i2c_client");
		return -EINVAL;
	}
	...
	sc6607_field_write(g_chip, F_PMID2OUT_OVP, 0x05);
	sc6607_voocphy_write_byte(chip->client, SC6607_REG_VAC_VBUS_OVP, 0x31); /*Vac ovp 11V	 Vbus_ovp 10V*/
	sc6607_voocphy_write_byte(chip->client, SC6607_REG_VBATSNS_OVP, 0x2E); /*VBAT_OVP:4.65V */
	sc6607_voocphy_write_byte(chip->client, SC6607_REG_IBUS_OCP_UCP, 0x6B); /* IBUS_OCP_UCP:3.75A */
	sc6607_voocphy_write_byte(chip->client, SC6607_REG_PHY_CTRL, 0x00); /*VOOC_CTRL:disable */
	...
	pr_err("sc6607 configed sc mode, ocp_reg=0x%02x", 0x6B);
	return 0;
}
...
static int sc6607_cp_config_bypass_mode(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_config_bypass_mode not get i2c_client");
		return -EINVAL;
	}
	...
	sc6607_voocphy_write_byte(chip->client, SC6607_REG_PHY_CTRL, 0x00); /*VOOC_CTRL:disable */
	chip->cp_work_mode = CP_WORKMODE_PPS;
	pr_err("sc6607 configed bypass mode");
	return 0;
}
...
static int sc6607_cp_get_ucp_flag(struct i2c_client *client)
{
	...
	ret = sc6607_voocphy_read_byte(chip->client, SC6607_REG_CP_FLT_FLG, &value);
	if (ret < 0) {
		pr_err("SC6607_REG_CP_FLT_FLG failed ret=%d\n", ret);
		return 0;
	}
	ucp_fail = (value & SC6607_VOOCPHY_IBUS_UCP_FALL_FLAG_MASK) >>
		   SC6607_VOOCPHY_IBUS_UCP_FALL_FLAG_SHIFT;
	pr_err("SC6607_REG_CP_FLT_FLG[0x%x] ucp_fail = %d\n", value, ucp_fail);
	return ucp_fail;
}
...
static int sc6607_set_cp_enable(struct i2c_client *client, int enable)
{
	...
	pr_err("sc6607 set cp %sabled", enable ? "en" : "dis");
	return ret;
}
...
static int sc6607_cp_get_vbus(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_get_vbus not get i2c_client");
		return -EINVAL;
	}
	...
	vbus = sc6607_voocphy_get_cp_vbus(chip);
...
static int sc6607_cp_get_ibus(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_get_ibus not get i2c_client");
		return -EINVAL;
	}
	...
	ibus = sc6607_voocphy_get_cp_ichg(chip);
...
static int sc6607_cp_get_vac(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_get_vac not get i2c_client");
		return -EINVAL;
	}

	ret = i2c_smbus_read_i2c_block_data(client, SC6607_REG_HK_VAC_ADC, 2, data_block);
	if (ret < 0) {
		sc6607_voocphy_i2c_error(true);
		pr_err("sc6607 read vac error \n");
	} else {
		sc6607_voocphy_i2c_error(false);
	}

	cp_vac = (((data_block[0] & SC6607_VOOCPHY_VAC_POL_H_MASK) << SC6607_VOOCPHY_VAC_POL_H_SHIFT) |
		    data_block[1]) * SC6607_VOOCPHY_VAC_ADC_LSB;
...
static int sc6607_cp_get_vout(struct i2c_client *client)
{
	...
	if (client == NULL) {
		pr_err("sc6607_cp_get_vout not get i2c_client");
		return -EINVAL;
	}

	ret = i2c_smbus_read_i2c_block_data(client, SC6607_REG_HK_VSYS_ADC, 2, data_block);
	if (ret < 0) {
		sc6607_voocphy_i2c_error(true);
		pr_err("sc6607 read vout error \n");
	} else {
		sc6607_voocphy_i2c_error(false);
	}

	cp_vout = (((data_block[0] & SC6607_VOOCPHY_VSYS_POL_H_MASK) << SC6607_VOOCPHY_VSYS_POL_H_SHIFT) |
		    data_block[1]) * SC6607_VOOCPHY_VSYS_ADC_LSB;
...
static int sc6607_cp_get_vbat(struct i2c_client *client)
{
	...
	vbat = oplus_sc6607_get_vbat(chip);
...
static int sc6607_cp_get_tdie(struct i2c_client *client)
{
	...
	ret = i2c_smbus_read_i2c_block_data(client, SC6607_REG_HK_TDIE_ADC, 2, data_block);
	if (ret < 0) {
		sc6607_voocphy_i2c_error(true);
		pr_err("sc6607 read vout error \n");
	} else {
	...
	cp_tdie = (((data_block[0] & SC6607_VOOCPHY_TDIE_POL_H_MASK) << SC6607_VOOCPHY_TDIE_POL_H_SHIFT) |
		(data_block[1] & SC6607_VOOCPHY_TDIE_POL_L_MASK)) *
	       SC6607_VOOCPHY_TDIE_ADC_LSB;
	if (cp_tdie < SC6607_VOOCPHY_TDIE_MIN || cp_tdie > SC6607_VOOCPHY_TDIE_MAX)
		cp_tdie = SC6607_VOOCPHY_TDIE_MAX;
...
irqreturn_t sc6607_protect_interrupt_handler(struct oplus_voocphy_manager *chip)
{
	...
	flag.value_bit.ibus_ucp = (value & SC6607_VOOCPHY_IBUS_UCP_FALL_FLAG_MASK) >>
		   SC6607_VOOCPHY_IBUS_UCP_FALL_FLAG_SHIFT;
	flag.value_bit.ibus_ocp = (value & SC6607_VOOCPHY_IBUS_OCP_FLAG_MASK) >>
		   SC6607_VOOCPHY_IBUS_OCP_FLAG_SHIFT;
	flag.value_bit.vbat_ovp = (value & SC6607_VOOCPHY_VBAT_OVP_FLAG_MASK) >>
		   SC6607_VOOCPHY_VBAT_OVP_FLAG_SHIFT;
	pr_err("%s SC6607_REG_CP_FLT_FLG[0x%x] flag = %d\n", __func__, value, flag);

	ret = sc6607_voocphy_read_byte(chip->client, SC6607_REG_HK_FLT_FLG, &value);
	if (ret < 0) {
		pr_err("SC6607_REG_HK_FLT_FLG failed ret=%d\n", ret);
		return 0;
	}
	flag.value_bit.vbus_ovp = (value & SC6607_HK_VBUS_OVP_MASK) >>
		   SC6607_HK_VBUS_OVP_SHIFT;
	pr_err("%s SC6607_REG_HK_FLT_FLG[0x%x] flag = %d\n", __func__, value, flag);

	ret = sc6607_voocphy_read_byte(chip->client, SC6607_REG_CHG_FLT_FLG, &value);
	if (ret < 0) {
		pr_err("SC6607_REG_CHG_FLT_FLG failed ret=%d\n", ret);
		return 0;
	}
	flag.value_bit.ibat_ocp = (value & SC6607_BUCK_IBAT_OCP_FLG_MASK) >>
		   SC6607_BUCK_IBAT_OCP_FLG_SHIFT;
	flag.value_bit.vout_ovp = (value & SC6607_BUCK_VSYS_OVP_FLG_MASK) >>
		   SC6607_BUCK_VSYS_OVP_FLG_SHIFT;
	pr_err("%s SC6607_REG_CHG_FLT_FLG[0x%x] flag = %d\n", __func__, value, flag);
...
static int sc6607_pps_check_and_register(struct oplus_voocphy_manager *chip)
{
	...
	if (index < CP_MAX && index >= CP_MASTER) {
		...
		if (of_property_read_string(node, "oplus,pps_dev-name", &sc6607_cp->dev_name)) {
			if (index != CP_MASTER) {
				pr_err("Warn: there is a nameless non-master cp");
				sc6607_cp->dev_name = "default_nameless_cp";
			} else {
				pr_err("Can't register a nameless master cp");
				return -ENODEV;
			}
		}
...
static int sc6607_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&chip->bc12_redetect_work, sc6607_bc12_redetect_work_func);
