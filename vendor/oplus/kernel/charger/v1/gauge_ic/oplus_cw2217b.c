...
static unsigned char config_profile_info[][SIZE_OF_PROFILE] = {
	{
		...
		0xAE, 0xB0, 0xB5, 0xB9, 0xA6, 0xA0, 0xE2, 0xCE,
		0xC5, 0xFF, 0xEB, 0xA4, 0x7E, 0x67, 0x58, 0x4D,
		0x46, 0x44, 0x41, 0xB1, 0x9D, 0xDF, 0xF6, 0xFD,
		0xD6, 0xD0, 0xD6, 0xD7, 0xD7, 0xD3, 0xCF, 0xCA,
		0xC5, 0xC3, 0xD1, 0xBC, 0x98, 0x8C, 0x84, 0x77,
		0x69, 0x6B, 0x80, 0x92, 0xAA, 0x8B, 0x6F, 0x61,
		0x20, 0x00, 0xAB, 0x10, 0x00, 0x91, 0x66, 0x00,
		0x00, 0x00, 0x64, 0x17, 0xA0, 0x87, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x21,
	},
	{
		0x50, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0xBB, 0xA9, 0xB9, 0xBC, 0xA6, 0xA0, 0xE1, 0xD0,
		0xC5, 0xFF, 0xF3, 0xA0, 0x85, 0x6C, 0x58, 0x4B,
		0x43, 0x3F, 0x36, 0xAF, 0x9D, 0xDF, 0x92, 0xE4,
		0xD2, 0xCD, 0xD2, 0xD6, 0xD5, 0xD3, 0xCF, 0xCC,
		0xC7, 0xC5, 0xD0, 0xC2, 0x9D, 0x91, 0x88, 0x82,
		0x75, 0x7A, 0x86, 0x8F, 0xA5, 0x96, 0x5F, 0x6C,
		0x20, 0x00, 0xAB, 0x10, 0x00, 0x91, 0x7E, 0x00,
		0x00, 0x00, 0x64, 0x17, 0xA0, 0x9B, 0x00, 0x00,
...
static void cw_get_battery_id(struct cw_battery *cw_bat)
{
	int ret = ERR_NUM;
	int value = NUM_0;
	...
	if (!cw_bat->batt_id_chan) {
		...
		if (ret < NUM_0 && !cw_bat->batt_id_chan) {
	...
	if (value >= BATID_LOW_MV1 && (value <= BATID_HIGH_MV1)) {
		cw_bat->batt_id = BAT_TYPE_LIWINON;
	} else if (value >= BATID_LOW_MV2 && (value <= BATID_HIGH_MV2)) {
		cw_bat->batt_id = BAT_TYPE_ATL;
	} else {
		cw_bat->batt_id = BAT_TYPE_UNKNOWN;
	}
	chg_err("[OPLUS_CHG][%s]:battery_id:%d\n", __func__, cw_bat->batt_id);
}
...
static int cw_config_start_ic(struct cw_battery *cw_bat)
{
	...
	if (cw_bat->batt_id != BAT_TYPE_UNKNOWN) {
		ret = cw_write_profile(cw_bat->client, config_profile_info[cw_bat->batt_id-1]);
		if (ret < NUM_0)
			return ret;
	} else {
		ret = cw_write_profile(cw_bat->client, config_profile_info[NUM_0]);
		if (ret < NUM_0)
			return ret;
	}
...
static int cw2217_get_state(struct cw_battery *cw_bat)
{
	...
	for (i = NUM_0; i < SIZE_OF_PROFILE; i++) {
		...
		if (cw_bat->batt_id != BAT_TYPE_UNKNOWN) {
			...
			if (config_profile_info[BAT_TYPE_UNKNOWN][i] != reg_val)
				break;
		}
	}
...
static void cw2217_parse_dt(struct cw_battery *cw_bat)
{
	...
	if (!node) {
		chg_err("%s device tree info. missing\n", __func__);
		return;
	}

	rc = of_property_read_u32(node, "cw2217,battery-fcc", &cw_bat->battery_fcc);
	if (rc) {
		chg_err("%s no battery fcc, use default value\n", __func__);
		cw_bat->battery_fcc = DESIGN_CAPACITY;
	}
}
