...
static int oplus_ufcs_check_bcc_curr(struct oplus_ufcs_chip *chip)
{
	...
	for (index_min = 0; index_min < chip->batt_curves.batt_curve_num; index_min++) {
		ufcs_debug(" [%d, %d, %d, %d, %d, %d, %d]\n", index_vbat, index_min,
	...
	for (index_max = index_min + 1; index_max < chip->batt_curves.batt_curve_num; index_max++) {
		ufcs_debug(" index_max index_vbat = %d,  [%d, %d, %d, %d, %d, %d]\n", index_vbat, index_max,
...
static int oplus_ufcs_get_batt_curve_curr(struct oplus_ufcs_chip *chip)
{
	...
	ufcs_debug(" [%d, %d, %d, %d, %d] [%d, %d, %d, %d, %d]\n", curve_volt_change, curve_time_change,
		   chip->batt_curve_index, chip->data.ap_batt_volt, oplus_ufcs_get_curve_vbat(chip),
	...
	if ((curve_volt_change) || (curve_time_change)) {
		...
		if ((chip->batt_curve_index > 0) &&
		    (oplus_ufcs_get_last_curve_vbat(chip) != oplus_ufcs_get_curve_vbat(chip)))
			oplus_ufcs_check_bcc_curr(chip);
...
static int oplus_ufcs_get_target_current(struct oplus_ufcs_chip *chip)
{
	...
	/*
          target_current_temp = target_current_temp < chip->ilimit.current_bcc ?
                                        target_current_temp :
...
static void oplus_ufcs_check_mos_btb(struct oplus_ufcs_chip *chip)
{
	if (chip->ops->ufcs_get_mos_temp_status && chip->ops->ufcs_get_mos_temp_status()) {
		...
		if (chip->count.mos_high >= UFCS_MOS_TEMP_OV_CNT) {
			...
			oplus_ufcs_track_upload_err_info(chip, UFCS_STOP_VOTER_MOS_OVER, false);
		}
	} else {
		chip->count.mos_high = 0;
	}
...
static void oplus_ufcs_check_preliminary_resistense(struct oplus_ufcs_chip *chip)
{
	...
	if (r_cool_down <= OPLUS_UFCS_CURRENT_LIMIT_4A)
...
static void oplus_ufcs_check_circuit_path(struct oplus_ufcs_chip *chip)
{
	...
	if (!chip || !chip->ops || !chip->ops->ufcs_get_mos0_switch || !chip->ops->ufcs_set_mos0_switch ||
...
void oplus_ufcs_set_bcc_current(int val)
{
	...
	ufcs_debug("[%d, %d, %d]\n", chip->ufcs_bcc_support, chip->ilimit.current_bcc, val);
