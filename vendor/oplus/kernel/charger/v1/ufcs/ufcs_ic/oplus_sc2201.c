...
static int __init sc2201_i2c_driver_init(void)
{
	int ret = 0;

	if (i2c_add_driver(&sc2201_i2c_driver) != 0) {
		ufcs_err(" failed to register sc2201 i2c driver.\n");
	} else {
		ufcs_err(" Success to register sc2201 i2c driver.\n");
	}

	return ret;
}

subsys_initcall(sc2201_i2c_driver_init);
#else
