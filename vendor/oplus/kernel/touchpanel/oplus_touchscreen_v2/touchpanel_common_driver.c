...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
static int ts_oplus_panel_notifier_callback(struct notifier_block *self,
				unsigned long event, void *data);
#endif
...
static void tp_gesture_handle(struct touchpanel_data *ts)
{
	...
	if (ts->health_monitor_support) {
		tp_healthinfo_report(&ts->monitor_data, HEALTH_GESTURE_READ,
...
static inline void tp_touch_handle(struct touchpanel_data *ts)
{
	...
	if(tp_debug == 1 && ts->monitor_data.RATE_MIN) {
		//TP_INFO(ts->tp_index, "tp_debug is %d,RATE_MIN is %d\n",tp_debug,ts->monitor_data.RATE_MIN );
	...
	if ((obj_attention & TOUCH_BIT_CHECK) != 0) {
		ts->up_status = false;
		//TP_INFO(ts->tp_index, "obj_attention is %d\n", obj_attention);
		for (i = 0; i < ts->max_num; i++) {
			//TP_INFO(ts->tp_index, " points[%d].status is %d points[i].x = %d,points[i].y = %d\n",i,points[i].status,points[i].x,points[i].y);
			if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
			    && (points[i].status == 0)) { /* buf[0] == 0 is wrong point, no process*/
			    //TP_INFO(ts->tp_index, " obj_attention & TOUCH_BIT_CHECK >> i\n");
				continue;
			}
			if (points[i].x > ts->resolution_info.max_x - 1
				...
				tp_healthinfo_report(&ts->monitor_data, HEALTH_REPORT, "x_y_over_max");
				 TP_INFO(ts->tp_index, " tp_healthinfo_report ++\n");
				continue;
			}

			if (((obj_attention & TOUCH_BIT_CHECK) >> i) & 0x01
			    && (points[i].status != 0)) {
			    //TP_INFO(ts->tp_index, "input_mt_slot ++\n");
				input_mt_slot(ts->input_dev, i);
				...
				//TP_INFO(ts->tp_index, "input_mt_report_slot_state ++\n");
				input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 0);
			}
		}
		...
		if (touch_near_edge ==
			...
			//TP_INFO(ts->tp_index, "touch_near_edge == finger_num\n");
			ts->view_area_touched = 0;
		}

	} else {
		if (ts->up_status) {
			//TP_INFO(ts->tp_index, "up_status is %d\n",ts->up_status);
			tp_touch_up(ts);
			mutex_unlock(&ts->report_mutex);
			return;
		}
		...
		for (i = 0; i < ts->max_num; i++) {
			//TP_INFO(ts->tp_index,"input_mt_slot and input_mt_report_slot_state ++\n");
			input_mt_slot(ts->input_dev, i);
			input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, 0);
	...
	if (ts->health_monitor_support) {
		//TP_INFO(ts->tp_index,"health_monitor_support is %d\n",ts->health_monitor_support);
		ts->monitor_data.touch_points = points;
...
static inline void tp_work_func(struct touchpanel_data *ts)
{
	...
	//TP_INFO(ts->tp_index,"tp_work_func ++\n");
	...
	if (ts->ts_ops->trigger_reason) {
		...
		//TP_INFO(ts->tp_index,"cur_event is 0x%4x\n", cur_event);
	}

	//TP_INFO(ts->tp_index,"tp_touch_handle ++\n");
...
/*
	...
	//snprintf(data_buf, 32, "firmware-data-0");
...
static int tp_paneldata_init(struct touchpanel_data *pdata)
{
	...
	ts->panel_data.test_limit_name = kzalloc(100, GFP_KERNEL|GFP_DMA);
	...
	//tp_util_get_vendor(&ts->panel_data);
...
static int ts_check_panel_dt(struct device *dev, struct touchpanel_data *ts)
{
	...
	if (!np) {
		...
		if (ts->tp_index == 0) {
			strncpy(disp_node, "qcom,dsi-display-primary", sizeof("qcom,dsi-display-primary"));
			TP_INFO(ts->tp_index,"si-display-primary ++\n");
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
	...
	TP_INFO(ts->tp_index, "ts_check_panel_dt is called\n");
	...
	if (ts->int_mode == UNBANNABLE) {
		TP_INFO(ts->tp_index, "tp_register_irq_func ++.\n");
	...
	if (strcmp(ts->touch_environment, "tvm") != 0) {
		...
		if (is_ftm_boot_mode(ts)) {
			TP_INFO(ts->tp_index, "is_ftm_boot_mode ++.\n");
...
#if IS_ENABLED(CONFIG_DRM_OPLUS_PANEL_NOTIFY)
		TP_INFO(ts->tp_index, "fb_notifier_callback ++\n");
...
#elif IS_ENABLED(CONFIG_QCOM_PANEL_EVENT_NOTIFIER)
		if (ts->active_panel) {
			TP_INFO(ts->tp_index, "ts_panel_notifier_callback ++\n");
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	ts->dsiplay_touch_notif.notifier_call = ts_oplus_panel_notifier_callback;
	ret = oplus_tp_panel_notifier_register_client(&ts->dsiplay_touch_notif);

	if (ret) {
		TP_INFO(ts->tp_index, "Unable to register oplus_touch_notifier: %d\n", ret);
		goto oplus_touch_notifier_fail;
	}
...
oplus_touch_notifier_fail:

#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	 oplus_tp_panel_notifier_unregister_client(&ts->dsiplay_touch_notif);
#endif
...
{
	struct touchpanel_data *ts = pdata;
#if IS_ENABLED(CONFIG_FB) || IS_ENABLED(CONFIG_DRM_MSM) || IS_ENABLED(CONFIG_DRM_OPLUS_NOTIFY) || IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	int ret;
#endif
	...
	if (ts->dsiplay_touch_notif.notifier_call) {
		ret = oplus_tp_panel_notifier_unregister_client(&ts->dsiplay_touch_notif);
		if (ret) {
			TP_INFO(ts->tp_index, "oplus_tp_panel_notifier_unregister_client: %d\n", ret);
		}
	}
#endif
...
static void speedup_resume(struct work_struct *work)
{
	...
	/*nvt36672 ic lcd and TP only one reset tp reset is update fw, lcd resume need update fw*/
	if (ts->resume_tp_update_fw) {
		ts->ts_ops->reset(ts->chip_data);
	}
...
{
	TP_INFO(ts->tp_index,"tp_resume_order : %d\n",ts->tp_suspend_order);
	if (ts->tp_suspend_order == TP_LCD_SUSPEND) {
		TP_INFO(ts->tp_index,"TP_LCD_SUSPEND ++");
		tp_suspend(ts->dev);
	} else if (ts->tp_suspend_order == LCD_TP_SUSPEND) {
		TP_INFO(ts->tp_index,"LCD_TP_SUSPEND ++");
		tp_suspend(ts->dev);
	}
};
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	case DRM_PANEL_EVENT_UNBLANK:
		if (notification->notif_data.early_trigger) {
			queue_work(ts->lcd_trigger_load_tp_fw_wq, &ts->fw_update_work);
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
static int ts_oplus_panel_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	TP_INFO(ts->tp_index, "gki notifier event is %d,blank is %d\n", event, evdata->blank);

	if (ts) {
		if (event == OPLUS_PANEL_EVENT_BLANK) {
			if (evdata->blank == OPLUS_PANEL_POWER_ON) {
				lcd_on_event(ts);
			} else if (evdata->blank == OPLUS_PANEL_POWER_OFF) {
				lcd_off_event(ts);
			} else if (evdata->blank == OPLUS_PANEL_CS_POWER_OFF) {
				TP_INFO(ts->tp_index,"%s: ESD cs and vddio low \n", __func__);
				tp_control_cs_gpio(0, ts->tp_index);  // cs  power off
			} else if (evdata->blank == OPLUS_PANEL_CS_POWER_ON) {
				TP_INFO(ts->tp_index,"%s: cs and vddio high \n", __func__);
				tp_control_cs_gpio(1, ts->tp_index);  // cs  power on
			}
		}
	}
