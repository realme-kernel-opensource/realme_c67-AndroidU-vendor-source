...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	static struct oplus_panel_notify_data touch_black_test_data;
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
		if (value == 1) {
			touch_black_test_data.gesture_flag = OPLUS_TOUCH_GESTURE_ON;
			oplus_tp_panel_notifier_call_chain(OPLUS_TOUCH_EVENT_BLANK,&touch_black_test_data);
		} else if (value == 0) {
			touch_black_test_data.gesture_flag = OPLUS_TOUCH_GESTURE_OFF;
			oplus_tp_panel_notifier_call_chain(OPLUS_TOUCH_EVENT_BLANK,&touch_black_test_data);
		}
#endif
...
static ssize_t proc_gesture_control_indep_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ret <= 0) {
		TPD_INFO("getting value failed, ret=%d\n", ret);
		return count;
	}
					...
					const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	ret = sscanf(buf, "%x,%d", (uint32_t *)&addr, &length);

	if(ret <= 0) {
		TPD_INFO("getting length failed, ret=%d\n", ret);
		return count;
	}
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
		touch_black_test_data.black_test_flag = OPLUS_TOUCH_BLACK_TEST_OFF;
		oplus_tp_panel_notifier_call_chain(OPLUS_TOUCH_EVENT_BLANK,&touch_black_test_data);
#endif
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	if (value == 1) {
		touch_black_test_data.black_test_flag = OPLUS_TOUCH_BLACK_TEST_ON;
		oplus_tp_panel_notifier_call_chain(OPLUS_TOUCH_EVENT_BLANK,&touch_black_test_data);
	}
#endif
