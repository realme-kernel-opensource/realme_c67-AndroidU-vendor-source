				       ...
				       int is_suspended)
{
	...
	if (ret) {
		...
		if (chip_info->point_data[1] == 0xFE) {
			TPD_INFO("Recover for fw reset %02X,++ IRQ_EXCEPTION\n", chip_info->point_data[1]);
			nvt_sw_reset_idle(chip_info);
		}
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	ts->monitor_data.is_gesture_waiting_read = true;
	ts->monitor_data.gesture_waiting = gesture->gesture_type;
	ts->monitor_data.gesture_received_time = ktime_to_ms(ktime_get());
