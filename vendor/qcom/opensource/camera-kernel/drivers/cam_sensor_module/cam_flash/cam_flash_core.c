	...
	bool regulator_enable)
{
	...
	if (regulator_enable && (fctrl->is_regulator_enabled == false)) {
		...
		if (fctrl->pmic_sc6607) {
			CAM_INFO(CAM_FLASH, "[SC6607] NOP power up settings.");
		...
		(fctrl->is_regulator_enabled == true)) {
			...
			if (fctrl->pmic_sc6607) {
				CAM_INFO(CAM_FLASH, "[SC6607] NOP power down settings.");
...
int cam_flash_on(struct cam_flash_ctrl *flash_ctrl,
	...
	int rc = 0;
	if (mode == 0) {
		if (flash_ctrl->flash_type == CAM_FLASH_TYPE_GPIO) {
...
int cam_flash_i2c_pkt_parser(struct cam_flash_ctrl *fctrl, void *arg)
{
	...
	case CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN_XTS:
		fctrl->sc6607_boost_xts = true;
		CAM_INFO(CAM_FLASH, "[SC6607] CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN_XTS E");

	case CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN:
		CAM_INFO(CAM_FLASH, "[SC6607] CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN E");
		fctrl->sc6607_boost = true;
		if (!fctrl->sc6607_boost_xts) {
			oplus_chg_set_camera_on(1);
			CAM_INFO(CAM_FLASH, "[SC6607] oplus_chg_set_camera_on CALL OPEN E");
		}
		CAM_INFO(CAM_FLASH, "[SC6607] oplus_chg_set_camera_on CALL OPEN X");
...
update_req_mgr:
	...
	if (((csl_packet->header.op_code  & 0xFFFFF) ==
		...
		((csl_packet->header.op_code & 0xFFFFF) ==
		CAM_FLASH_PACKET_OPCODE_NEED_BOOST_EN_XTS)) {
