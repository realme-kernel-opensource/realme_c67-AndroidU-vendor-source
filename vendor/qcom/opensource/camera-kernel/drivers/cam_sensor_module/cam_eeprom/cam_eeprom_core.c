...
#include "cam_eeprom_beforeread.h"
#define MAX_READ_SIZE  0x7FFFF
#define hi846w_i2c_slave_addr 0x40
#define HI846W_OTP_DATA_LEN   0x1587
#define OTP_DATA_LEN          0x2000
uint8_t OTPData[OTP_DATA_LEN];
/**
	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (soc_private->i2c_info.slave_addr == 0x40){
			...
			if(!eeprom_memory_map_unint_para(eb_info->i2c_info.slave_addr,e_ctrl)){
				CAM_DBG(CAM_EEPROM,"unint otp pata data successfully");
			}else{
...
int eeprom_memory_map_read_data(uint32_t slave_addr,struct cam_eeprom_memory_map_t emap,
	...
	int length = 0 , eeprom_result = 0;
	uint32_t  count_write , count_read;
	struct cam_sensor_i2c_reg_setting  i2c_reg_settings = {0};
	struct cam_sensor_i2c_reg_array    i2c_reg_array = {0};
	struct eeprom_memory_map_init_write_params *pWriteParams = NULL;
	struct OplusOtpCheckInfo *pOtpCheckInfo = NULL;
	eeprom_result = eeprom_process_para_before_read(slave_addr,&pWriteParams,&pOtpCheckInfo,&count_read);
	if(eeprom_result < 0){
	...
	for(length = 0; length <count_read; length++) {
		eeprom_result = camera_io_dev_read_seq(&e_ctrl->io_master_info,
			emap.mem.addr, &OTPData[length],
			emap.mem.addr_type,
	...
	eeprom_result = eeprom_memory_select_group(pOtpCheckInfo);
	if (eeprom_result < 0) {
		CAM_ERR(CAM_EEPROM, "failed to select group",eeprom_result);
	}
	eeprom_result = eeprom_memory_process_otp_data(pOtpCheckInfo,memptr);
	if (eeprom_result < 0) {
		CAM_ERR(CAM_EEPROM, "failed to process otp data",eeprom_result);
	}
	return eeprom_result;
}
int eeprom_process_para_before_read(uint32_t slave_addr,struct eeprom_memory_map_init_write_params **pWriteParams,
									struct OplusOtpCheckInfo **pOtpParams,uint32_t *count_read){
	int eeprom_result = 0;
	if(hi846w_i2c_slave_addr == slave_addr){
		CAM_DBG(CAM_EEPROM, "hi846 get otp init paras");
		*pWriteParams = &hi846w_eeprom;
		*pOtpParams = &hi846w_otp_checkinfo;
		*count_read = (uint32_t)HI846W_OTP_DATA_LEN;
...
int eeprom_memory_select_group(struct OplusOtpCheckInfo *pOtpParams){
	...
	int eeprom_result = 0;
	if(!pOtpParams){
		CAM_ERR(CAM_EEPROM, "pOtpParams is NULL");
		eeprom_result = -1;
		return eeprom_result;
	}
	if (pOtpParams->groupInfo.IsAvailable) {
		for( module=0; module < CHECKNUMMAX; module++) {
			for( group=0; group < GROUPNUMMAX; group++) {
				if (OTPData[ pOtpParams->groupInfo.CheckItemOffset[module][group]] == pOtpParams->groupInfo.GroupFlag) {
					CAM_DBG(CAM_EEPROM, "part:select group: %d.", group);
					CAM_DBG(CAM_EEPROM, "OTPData[module]:%d,GroupFlag:%d",
										OTPData[ pOtpParams->groupInfo.CheckItemOffset[module][group]],
										pOtpParams->groupInfo.GroupFlag);
					pOtpParams->groupInfo.SelectGroupNum[module] = group ;
					break;
				}else{
					CAM_DBG(CAM_EEPROM, "OTPData[module]:%d,GroupFlag:%d",
										OTPData[ pOtpParams->groupInfo.CheckItemOffset[module][group]],
		...
		if( group == GROUPNUMMAX ) {
			CAM_ERR(CAM_EEPROM, "part: %d bad otp data.", group);
			pOtpParams->groupInfo.SelectGroupNum[module] = GROUPNUMMAX;
		}
	}
	return eeprom_result;
}
...
int eeprom_memory_process_otp_data(struct OplusOtpCheckInfo *pOtpParams,uint8_t *memptr){
	...
	if(!pOtpParams){
		CAM_ERR(CAM_EEPROM, "Params is NULL");
		eeprom_result = -1;
		return eeprom_result;
	}
	for(module=0; module<CHECKNUMMAX; module++){
		SelectGroupTemp = pOtpParams->groupInfo.SelectGroupNum[module];
		GroupOffset = pOtpParams->ItemInfo[module][SelectGroupTemp].Offset;
		GroupLenth = pOtpParams->ItemInfo[module][SelectGroupTemp].Length;
		CAM_ERR(CAM_EEPROM, "SelectGroupTemp:%d, GroupOffset:%d, GroupLenth :%d",
							SelectGroupTemp, GroupOffset, GroupLenth);
		if(1 == pOtpParams->ItemInfo[module][SelectGroupTemp].IsAvailable){
			CAM_ERR(CAM_EEPROM, " eeprom_memory_process_otp_data copy data");
			memcpy(memptr,&OTPData[GroupOffset],GroupLenth);
			memptr += GroupLenth;
		}
	}
...
int eeprom_process_para_after_read(uint32_t slave_addr,struct eeprom_memory_map_init_write_params **pWriteParams){
	...
	if(hi846w_i2c_slave_addr == slave_addr){
		...
		*pWriteParams = &hi846w_eeprom_after_read;
	}else{
		eeprom_result = -1;
		CAM_DBG(CAM_EEPROM, "it is not a valid otp");
	}
