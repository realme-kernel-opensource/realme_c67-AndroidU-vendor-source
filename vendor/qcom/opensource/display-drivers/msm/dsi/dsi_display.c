...
/*samir.liu@RM.MM.Display.LCD.bsp,2023-8-23,add funtion*/
#include "../oplus/oplus_display_feature.h"
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
#define BLACK_TEST_OR_GESTURE_ON  true
#define BLACK_TEST_OR_GESTURE_OFF  false
bool g_touch_black_flag = BLACK_TEST_OR_GESTURE_OFF;
bool g_touch_gesture_flag = BLACK_TEST_OR_GESTURE_OFF;
static struct notifier_block touch_notif_lcd;
static int nvt_oplus_touch_notifier_callback(struct notifier_block *self,
		...
		void *display, u32 bl_lvl)
{
	...
	if (strstr(dsi_display->name, "mdss_dsi_sim_video")) {
		DSI_ERR("[%s] The current screen is a virtual panel, skip set backlight\n",
				__func__);
		return rc;
	}
...
static bool dsi_display_validate_reg_read(struct dsi_panel *panel)
{
	...
	struct oplus_display_feature *oplus_feature = get_oplus_display_feature();
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
			if (config->return_buf[i] !=
				...
				DRM_ERROR("esd mismatch: 0x%x, expected:0x%x\n",
						config->return_buf[i], config->status_value[group + i]);
				oplus_feature->oplus_esd_flag = true;
				DRM_DEBUG("oplus_feature->oplus_esd_flag:%d \n", oplus_feature->oplus_esd_flag);
				break;
			} else {
				oplus_feature->oplus_esd_flag = false;
				DRM_DEBUG("oplus_feature->oplus_esd_flag:%d \n", oplus_feature->oplus_esd_flag);
			}
		}
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
static int nvt_oplus_touch_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{

	struct oplus_panel_notify_data  *evdata = data;

	if (!evdata)
		return 0;

	DSI_INFO( "gki notifier event:%d,black_test_flag:%d\n", event, evdata->black_test_flag);

		if (event == OPLUS_TOUCH_EVENT_BLANK) {
			if (evdata->black_test_flag == OPLUS_TOUCH_BLACK_TEST_ON) {
				DSI_INFO( "black_test_on:%d\n", evdata->black_test_flag);
				g_touch_black_flag = BLACK_TEST_OR_GESTURE_ON;
			} else if (evdata->black_test_flag == OPLUS_TOUCH_BLACK_TEST_OFF) {
				DSI_INFO( "black_test_off:%d\n", evdata->black_test_flag);
				g_touch_black_flag = BLACK_TEST_OR_GESTURE_OFF;
			}
			if (evdata->gesture_flag == OPLUS_TOUCH_GESTURE_ON) {
				DSI_INFO( "gesture:%d\n", evdata->gesture_flag);
				g_touch_gesture_flag = BLACK_TEST_OR_GESTURE_ON;
			} else if (evdata->gesture_flag == OPLUS_TOUCH_GESTURE_OFF) {
				DSI_INFO( "gesture:%d\n", evdata->gesture_flag);
				g_touch_gesture_flag = BLACK_TEST_OR_GESTURE_OFF;
			}
		}
...
unsigned int cabc_gear_backup = 0x03;
static unsigned int disp_param_value = 0;
static struct kobject *display_kobj;
static struct dsi_display *g_dsi_display;

static ssize_t disp_param_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf)
{
	ssize_t ret = 0;
	DSI_INFO("[drm] %s\n", __func__);

	ret = snprintf(buf, sizeof(disp_param_value), "%d\n", disp_param_value);

	return ret;
}

static ssize_t disp_param_store(struct kobject *kobj, struct kobj_attribute *attr,
			const char *buf, size_t num)
{
	struct dsi_display *display = g_dsi_display;

	DSI_INFO("[drm] %s\n", __func__);

	if(!display || !display->panel) {
		DSI_ERR("%s: display or panel is a NULL pointer\n", __func__);
		return num;
	}
	...
	if (!dsi_panel_initialized(display->panel)) {
		DSI_ERR("%s: panel is not initialized\n", __func__);
		dsi_panel_release_panel_lock(display->panel);
		return num;
	}

	sscanf(buf, "%x", &disp_param_value);
	DSI_INFO("%s: input disp_param_value=0x%x, is panel initialized: %d\n", __func__, disp_param_value, display->panel->panel_initialized);

	switch(disp_param_value & 0x0f)
	{
		case 0x01:
			dsi_panel_tx_cmd_set(display->panel, DSI_CMD_SET_CABC_UI_ON);
			DSI_INFO("[drm] set cabc 1 gear\n");
			cabc_gear_backup = 0x01;
			break;

		case 0x02:
			dsi_panel_tx_cmd_set(display->panel, DSI_CMD_SET_CABC_STILL_ON);
			DSI_INFO("[drm] set cabc 2 gear\n");
			cabc_gear_backup = 0x02;
			break;

		case 0x03:
			dsi_panel_tx_cmd_set(display->panel, DSI_CMD_SET_CABC_MOVIE_ON);
			DSI_INFO("[drm] set cabc 3 gear\n");
			cabc_gear_backup = 0x03;
			break;

		case 0x04:
			dsi_panel_tx_cmd_set(display->panel, DSI_CMD_SET_CABC_OFF);
			DSI_INFO("[drm] set cabc 0 gear\n");
			cabc_gear_backup = 0x04;
			break;
...
static ssize_t lcm_name_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf)
{
	ssize_t ret = 0;
	struct dsi_display *display = g_dsi_display;
	...
	if(!display || !display->name) {
		DSI_ERR("%s: display or display name is a NULL pointer\n", __func__);
		return ret;
	}

	ret = snprintf(buf, strlen(display->name)+1, "%s\n", display->name);

	return ret;
}
static struct kobj_attribute dev_attr_disp_param =
	__ATTR(disp_param, 0644, disp_param_show, disp_param_store);
static struct kobj_attribute dev_attr_lcm_name =
	__ATTR(lcm_name, 0644, lcm_name_show, NULL);


static struct attribute *panel_attributes[] = {
	&dev_attr_disp_param.attr,
	&dev_attr_lcm_name.attr,
	NULL
};

static struct attribute_group panel_attribute_group = {
	.attrs = panel_attributes
};

static int panel_create_sysfs(void)
{
	int ret = 0;
	...
	ret = sysfs_create_group(display_kobj, &panel_attribute_group);
	if (ret) {
		DSI_ERR("[drm]: sysfs_create_group() failed!!\n");
		sysfs_remove_group(display_kobj, &panel_attribute_group);
		return -ENOMEM;
	} else {
		DSI_INFO("[drm]: sysfs_create_group() succeeded!!\n");
	}

	return ret;
}

static int panel_remove_sysfs(void)
{
	int ret = 0;
	if(display_kobj) {
		sysfs_remove_group(display_kobj, &panel_attribute_group);
		DSI_INFO("[drm]: sysfs_remove_group() succeeded!!\n");
	}
...
int dsi_display_dev_probe(struct platform_device *pdev)
{
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	touch_notif_lcd.notifier_call = nvt_oplus_touch_notifier_callback;
	rc = oplus_tp_panel_notifier_register_client(&touch_notif_lcd);

	if (rc) {
		DSI_ERR("oplus_tp_panel_notifier_register_client fail!\n");
		oplus_tp_panel_notifier_unregister_client(&touch_notif_lcd);
	}
	...
	if (!firm_req) {
        ...
        if (!strcmp(display->display_type, "primary")) {
            ...
            rc = oplus_display_feature_init(display);
            if (rc < 0) {
                DSI_ERR("[%s]:oplus display feature init faile!\n",__func__);
            }
        } else {
...
int dsi_display_dev_remove(struct platform_device *pdev)
{
	...
	if (touch_notif_lcd.notifier_call) {
		rc = oplus_tp_panel_notifier_unregister_client(&touch_notif_lcd);
		if (rc) {
			DSI_ERR("oplus_tp_panel_notifier_unregister_client: %d\n", rc);
		}
	}
#endif
