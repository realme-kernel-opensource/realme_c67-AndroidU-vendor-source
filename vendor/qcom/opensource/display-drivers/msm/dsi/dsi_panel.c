...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
static struct oplus_panel_notify_data lcd_notif_touch;
extern bool g_touch_black_flag;
extern bool g_touch_gesture_flag;
#endif
static void panel_enable_work_func(struct work_struct *work);
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if(strstr(panel->name, "mdss_dsi_sim_video")) {
		DSI_ERR("[%s] The current screen is a virtual panel, skip power on\n",
				panel->name);
		goto exit;
	}

	if(!panel->external_power_enable) {
...
#if IS_ENABLED(CONFIG_OCP2130_BIAS)
		panel_vddio_enable(1);
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
		lcd_notif_touch.blank = OPLUS_PANEL_CS_POWER_ON;
		oplus_tp_panel_notifier_call_chain(OPLUS_PANEL_EVENT_BLANK, &lcd_notif_touch);
		DSI_INFO("[TP] vddio and cs power on!");
#endif
		ocp2130_enable();
#endif
	}
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if(strstr(panel->name, "mdss_dsi_sim_video")) {
		DSI_ERR("[%s] The current screen is a virtual panel, skip power off\n",
				panel->name);
		return rc;
	}
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	if (((g_touch_black_flag == false) && (g_touch_gesture_flag == false)) || oplus_feature->oplus_esd_flag) {
		DSI_INFO("[TP] g_touch_black_flag:%d\n", g_touch_black_flag);
#endif
		...
		if (oplus_feature->oplus_esd_flag) {
			if (gpio_is_valid(panel->reset_config.reset_gpio) &&
							!panel->reset_gpio_always_on)
				gpio_set_value(panel->reset_config.reset_gpio, 0);
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
			lcd_notif_touch.blank = OPLUS_PANEL_CS_POWER_OFF;
			oplus_tp_panel_notifier_call_chain(OPLUS_PANEL_EVENT_BLANK, &lcd_notif_touch);
			DSI_INFO("[TP] esd cs power off!");
#endif

#if IS_ENABLED(CONFIG_OCP2130_BIAS)
			panel_vddio_enable(0);
#endif
		} else {
			if (gpio_is_valid(panel->reset_config.reset_gpio) &&
							!panel->reset_gpio_always_on)
	...
	u32 bl_lvl)
{
	...
	struct oplus_display_feature *display_feature = get_oplus_display_feature();
	u32 bl_lvl_backup = 0;
	...
	if(panel->silence_skip_bl_enable){
		DSI_INFO("silence_skip_bl_enable:%d\n", panel->silence_skip_bl_enable);
		return rc;
	}
	...
	if (!display_feature) {
		DSI_INFO("display_feature is a NULL pointer\n");
	} else {
		if (display_feature->oplus_display_pwm_update && display_feature->oplus_display_pwm_enable)
			display_feature->oplus_display_pwm_update(bl_lvl);

		if (display_feature->oplus_bl_maping) {
			bl_lvl = display_feature->oplus_bl_maping(bl_lvl);
		}
	}
	...
	if (panel->bl_config.bl_inverted_dbv) {
		DSI_INFO("%s bl_lvl=%d, brightness=%d\n", __func__, bl_lvl_backup, bl_lvl);
	...
	if (rc < 0)
		DSI_ERR("failed to update dcs backlight:0x%x\n", bl_lvl);
				...
				bool trusted_vm_env)
{
	...
	panel->external_power_enable = utils->read_bool(utils->data,
				"qcom,external_power_enable");
	...
	panel->panel_workqueue = create_singlethread_workqueue("panel_workqueue");
	INIT_DELAYED_WORK(&panel->panel_enable_work,panel_enable_work_func);
		       ...
		       struct mipi_dsi_host *host)
{
	...
	if (unlikely(MSM_BOOT_MODE__SILENCE == bootmode)) {
		DSI_INFO("silence boot, skip set backlight\n");
		panel->silence_skip_bl_enable = true;
	} else {
		panel->silence_skip_bl_enable = false;
	}
	goto exit;
...
extern unsigned int cabc_gear_backup;

static void restore_cabc_mode(struct dsi_panel *panel, unsigned int cabc_mode)
{
	switch(cabc_mode & 0x0f)
	{
		case 0x01:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_UI_ON);
			DSI_INFO("[drm] restore cabc 1 gear\n");
			break;

		case 0x02:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_STILL_ON);
			DSI_INFO("[drm] restore cabc 2 gear\n");
			break;

		case 0x03:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_MOVIE_ON);
			DSI_INFO("[drm] restore cabc 3 gear\n");
			break;

		case 0x04:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CABC_OFF);
			DSI_INFO("[drm] restore cabc 0 gear\n");
			break;

		default:
			DSI_INFO("[drm] restore cabc default\n");
			break;
	}
}
...
static void panel_enable_work_func(struct work_struct *work)
{
	struct dsi_panel *panel = container_of(work, struct dsi_panel, panel_enable_work.work);
	DSI_INFO("[drm] %s\n", __func__);
	if (!panel) {
		DSI_ERR("[drm] %s panel is null ptr\n", __func__);
		return;
	}
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
	lcd_notif_touch.blank = OPLUS_PANEL_POWER_ON;
	oplus_tp_panel_notifier_call_chain(OPLUS_PANEL_EVENT_BLANK, &lcd_notif_touch);
	DSI_INFO("[drm] display notifer touch resume!");
#endif
	return;
}
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	queue_delayed_work(panel->panel_workqueue, &panel->panel_enable_work, 0);
	...
	restore_cabc_mode(panel, cabc_gear_backup);
...
#if IS_ENABLED(CONFIG_OPLUS_TP_PANEL_NOTIFIER)
        lcd_notif_touch.blank = OPLUS_PANEL_POWER_OFF;
        oplus_tp_panel_notifier_call_chain(OPLUS_PANEL_EVENT_BLANK, &lcd_notif_touch);
        DSI_INFO("[TP]display notifer touch suspend!");
#endif
