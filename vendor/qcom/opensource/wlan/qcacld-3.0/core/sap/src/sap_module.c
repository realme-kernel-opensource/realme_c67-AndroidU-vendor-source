...
dfs_master_mode_check:
	 ...
	 * Limit SAP to 20Mhz if STA present.
	 */
	if (sta_present) {
		sap_debug("STA present, Limit SAP/GO to 20Mhz");
		return QDF_MIN(channel_width, sta_chan_width);
	}

	if (policy_mgr_mode_specific_connection_count(psoc,PM_STA_MODE, NULL) > 0) {
		sap_debug("MCC mode: set BW coming to 20MHZ");
		return QDF_MIN(channel_width, sta_chan_width);
    }
